...
//   Add a Note
  app.post("/add-note",(req,res)=>{
    ...
    connection.query(sql,[uuid.v4(),newNote.title,newNote.contents,new Date(newNote.timestamp)],(error,results)=>{
        if(error){
            console.error('Error executing MySQL query:', error);
            return res.status(500).json({ error: 'Internal Server Error' });
        }
...
//   Update a Note
  ...
  app.post("/update-note/:id", (req, res) => {
    ...
    connection.query(sql, [updatedNote.title, updatedNote.contents,new Date(updatedNote.timestamp) , noteId], (error, results) => {
        if (error) {
            console.error('Error executing MySQL query:', error);
            return res.status(500).json({ error: 'Internal Server Error' });
        }
